...
def analyze_enterprise(enterprise, metrics):
    """综合分析企业数据"""
    results = {}

    # 财务分析
    if 'financial' in metrics:
        results['financial'] = analyze_financial(enterprise)

    # 市场趋势分析
    if 'market_trend' in metrics:
        results['market_trend'] = analyze_market_trend(enterprise)

    # 竞争力分析
    if 'competitiveness' in metrics:
        results['competitiveness'] = analyze_competitiveness(enterprise)
...
if __name__ == '__main__':
    ...
    result = analyze_enterprise(enterprise, metrics)
